...
**Объектом исследования** является многокомпонентный конвейер видеоаналитики, включающий модули захвата видео, ML-обработку, брокер сообщений Kafka и backend-сервисы.
...
2.  **Выбор моделей:** Для сравнения были выбраны три модели, представляющие разные подходы:
    *   **SARIMA:** классическая статистическая модель.
...
3.  **Валидация:** Для корректной оценки качества применялась временная кросс-валидация (`TimeSeriesSplit`). Этот выбор критически важен для предотвращения **утечки данных** — ситуации, когда модель при обучении "подглядывает" в будущее. Первоначальные эксперименты показали, что методологическая ошибка в валидации может дать нереалистично завышенный результат с ошибкой менее 0.4%, что подчеркивает абсолютную необходимость строгого подхода к проверке моделей.
